  ...
  describe('Client Creation & Configuration', () => {
    ...
    describe('Custom Headers', () => {
      test('should have custom header set', () => {
        ...
        const getHeaders = (request as any).headers
        expect(getHeaders).toHaveProperty('X-Test-Header', 'value')
      })

      test('should merge custom headers with default headers', () => {
        const customHeader = { 'X-Test-Header': 'value' }
        const client = new TestableSupabaseClient(URL, KEY, { global: { headers: customHeader } })
        expect(client.headersPublic).toHaveProperty('X-Test-Header', 'value')
        expect(client.headersPublic).toHaveProperty('X-Client-Info')
      })
    })
