...
const { url: SUPABASE_URL, anonKey: ANON_KEY, jwtSecret: JWT_SECRET } = getConnectionInfo()
...
describe('Supabase Integration Tests', () => {
  ...
  describe.each([{ vsn: '1.0.0' }, { vsn: '2.0.0' }])('Realtime with vsn: $vsn', ({ vsn }) => {
    ...
    beforeEach(async () => {
      ...
      supabase = createRealtimeClient({ vsn })
...
describe('Storage API', () => {
  ...
  const supabaseWithServiceRole = createServiceRoleClient()
...
describe('Custom JWT', () => {
  describe('Realtime', () => {
    test('will connect with a properly signed jwt token', async () => {
      ...
      const supabaseWithCustomJwt = createRealtimeClient(
        {},
