import { SortByV2, StorageClient } from '@supabase/storage-js'
...
import {
  createStorageClient,
  createNewBucket,
  findOrCreateBucket as findOrCreateBucketHelper,
  getStorageInfo,
} from '../../helpers/storage-client'

const storage = createStorageClient()
const { url: URL } = getStorageInfo()
...
const findOrCreateBucket = async (name: string, isPublic = true) => {
  return findOrCreateBucketHelper(storage, name, isPublic)
}
