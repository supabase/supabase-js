...
- Periodically run `npx depcheck` (or the equivalent for your stack) and remove deps that aren't imported anywhere.
- Look at your direct deps when a CVE lands. Is the dep doing something you could do in 20 lines yourself? Some of the most-exploited packages are tiny utilities that became transitive footguns.
...
Prevention is only half the job — you also need to find out when something has gone wrong upstream, ideally before the news reaches Twitter.
