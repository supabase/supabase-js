...
describe('syncState', function () {
  test('syncs empty state', function () {
    let state = {}
  ...
  test("onJoins new presences and onLeave's left presences", function () {
  ...
  test('onJoins only newly added presences', function () {
...
describe('syncDiff', function () {
  ...
  test('removes presence when presences is empty and adds additional presence', function () {
  ...
  test('removes presence while leaving key if other presences exist', function () {
    let state = {
      u1: [
        { id: 1, presence_ref: '1' },
        { id: 1, presence_ref: '1.2' },
...
describe('instance', function () {
  test('syncs state and diffs', function () {
  ...
  test('applies pending diff if state is not yet synced', function () {
    const presence = new RealtimePresence(channelStub)
    const onJoins = []
    const onLeaves = []
  ...
  test('allows custom channel events', function () {
    const presence = new RealtimePresence(channelStub, {
      events: {
        state: 'the_state',
        diff: 'the_diff',
  ...
  test('updates existing presences for a presence update (leave + join)', function () {
    const presence = new RealtimePresence(channelStub)
    const onJoins = []
    const onLeaves = []
    ...
    assert.deepEqual(
      RealtimePresence.map(presence.state, (_id, presences) => presences),
      [
        [
          {
            id: 1,
