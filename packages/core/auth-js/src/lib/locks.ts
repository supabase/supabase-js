/**
 * Lock primitives retained for backwards-compatible imports. The v3 auth
 * client coordinates refreshes itself (deduping in-instance callers onto a
 * shared in-flight promise) and lets the GoTrue server resolve cross-instance
 * races, so it does not invoke any primitive from this module. The functions
 * still work for direct callers that need a navigator.locks-backed or
 * in-process exclusive lock of their own. Scheduled for removal in v4.
 */

import { supportsLocalStorage } from './helpers'

/**
 * @deprecated Debug flag for `navigatorLock` / `processLock`. The v3 auth
 ...
 *
 * @deprecated The v3 auth client doesn't acquire locks around auth
 * operations, so this error never originates from `supabase.auth.*` calls.
 * Direct callers of `navigatorLock` / `processLock` still receive it on
 * acquire timeout.
 */
...
/**
 * @deprecated The v3 auth client doesn't call `navigator.locks`, so this
 * error never originates from `supabase.auth.*` calls. Direct callers of
 * `navigatorLock` still receive it on acquire timeout.
 */
export class NavigatorLockAcquireTimeoutError extends LockAcquireTimeoutError {}
/**
 * @deprecated The v3 auth client doesn't run `processLock`, so this error
 ...
 *
 * @deprecated The v3 auth client coordinates refreshes itself and the
 * server resolves concurrent refresh races, so passing `{ lock: navigatorLock }`
