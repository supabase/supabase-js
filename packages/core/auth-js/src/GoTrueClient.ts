...
const DEFAULT_OPTIONS: Omit<
  ...
  'fetch' | 'storage' | 'userStorage' | 'lock' | 'lockAcquireTimeout'
> = {
  url: GOTRUE_URL,
...
export default class GoTrueClient {
  ...
  constructor(options: GoTrueClientOptions) {
    ...
    // compatibility and have no effect; deliberately not read here.
  ...
  /**
   * Receive a notification every time an auth event happens. Callbacks can be
   * `async` and can safely call other Supabase auth methods (`getUser`,
   * `setSession`, etc.) from inside the callback.
   *
   * @param callback A callback function to be invoked when an auth event happens.
   */
  ...
  private async _callRefreshToken(refreshToken: string): Promise<CallRefreshTokenResult> {
    ...
    try {
      ...
      // Commit guard: storage may have changed under us between fetch start
      // and now (usually because a concurrent signOut ran `_removeSession`).
      // If the refresh token we just rotated isn't in storage anymore,
      ...
      if (!currentStored || currentStored.refresh_token !== refreshToken) {
        this._debug(
          ...
          {
            had: currentStored ? `${currentStored.refresh_token?.substring(0, 5)}...` : null,
            expected: `${refreshToken.substring(0, 5)}...`,
          }
        )
   ...
   * still return whatever is currently persisted; dispose doesn't erase storage.
   *
