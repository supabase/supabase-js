...
export default class GoTrueClient {
   ...
   * @param factorID
   * @param code Takes in a code generated by an authenticator device
   */
  private async _unenroll(params: MFAUnenrollParams) {
    ...
    try {
      return await _request(this.fetch, 'POST', `${this.url}/user/${userID}/factor/${factorID}`, {
        body: {code: code},
        headers: this.headers,
      })
    } catch (error) {
  ...
  private async _enroll(params: MFAEnrollParams) {
    ...
    const userID = data.session?.user.ID
    try {
      return await _request(this.fetch, 'POST', `${this.url}/user/${userID}/factor`, {
        body: {...params},
        headers: this.headers,
      })
    } catch (error) {
   ...
   */
  private async _verify(params: MFAVerifyParams) {
    const { data, error: sessionError } = await this.getSession()
    if (sessionError) {
      return { error: sessionError }
    }
    ...
    const userID = data.session?.user.ID
    try {
      return await _request(this.fetch, 'POST', `${this.url}/user/${userID}/factor/${factorID}/verify`, {
        body: {code: code},
        headers: this.headers,
        redirectTo: options.redirectTo,
      })
  ...
  /**
   * Generates a challenge for a given factor
   * @param factorID
   */
  ...
  /**
   * Second step of two-factor login which allows user to be second factor authenticated.
   * @param factorID System assigned identifier for authenticator device as returned by enroll.
   * @param code Code Generated by an authenticator device.
   * @param recoveryCode Code assigned to user on first login which can be used to bypass MFA login.
   *
   */
  private async _login(params: MFALoginParams) {
    const { data, error: sessionError } = await this.getSession()
    if (sessionError) {
      return { error: sessionError }
    }
    const userID = data.session.user.ID
    const {factorID, code, recoveryCode} = params
    try {
      return await _request(this.fetch, 'POST', `${this.url}/user/${userID}/factor/${factorID}/login`, {
        body: {recoveryCode: recoveryCode, code: code},
        headers: this.headers,
        redirectTo: options.redirectTo,
      })
    } catch (error) {
  ...
  private async _listDevices() {
    ...
    // TOOO(Joel): Add factors to session
    return data.session.user.factors
  }
}
