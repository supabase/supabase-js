...
// 3. Run `npm run test:cors-validation` from test/deno
...
Deno.test(
  ...
  async (t) => {
    ...
    await t.step('OPTIONS preflight - should return 204 status', async () => {
    ...
    await t.step('OPTIONS preflight - should include all HTTP methods', async () => {
      ...
      const allowedMethodsUpper = allowedMethods.toUpperCase()
