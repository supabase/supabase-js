-- Enable PostgREST features for testing
...
ALTER TABLE public.channels REPLICA IDENTITY FULL; -- Send "previous data" to supabase
...
ALTER TABLE public.collections REPLICA IDENTITY FULL; -- Send "previous data" to supabase
...
-- SELF REFERENCING TABLE WITH MULTIPLE FOREIGN KEYS
CREATE TABLE public.lab (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name text,
  type text,
  parent bigint CONSTRAINT lab_parent_fkey REFERENCES public.lab(id) ON DELETE CASCADE,
  main bigint CONSTRAINT lab_main_fkey REFERENCES public.lab(id) ON DELETE CASCADE
);
...
-- Hotel and booking tables (for testing nullable foreign keys)
create table hotel (
  id bigint generated by default as identity primary key,
  name text null
);
...
-- Grant access to personal schema for PostgREST (must be after tables are created)
GRANT USAGE ON SCHEMA personal TO anon, authenticated;
GRANT ALL ON ALL TABLES IN SCHEMA personal TO anon, authenticated;
GRANT ALL ON ALL SEQUENCES IN SCHEMA personal TO anon, authenticated;
